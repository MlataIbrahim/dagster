...
def execute_start_command(schedule_name, all_flag, cli_args, print_fn):
    ...
    if all_flag:
        for schedule in instance.all_schedulesr(repository):
            try:
                schedule = instance.start_scheduler(repository, schedule.name)
            except DagsterInvariantViolationError as ex:
                raise click.UsageError(ex)
        ...
        try:
            schedule = instance.start_scheduler(repository, schedule_name)
        except DagsterInvariantViolationError as ex:
            raise click.UsageError(ex)
